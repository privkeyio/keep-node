...
A degraded RNG path must never succeed quietly. The only key material this repo generates itself is the LUKS bootstrap passphrase for the vault data volume, drawn during first-boot provisioning in `nixos/frost-gate.nix`; everything else (FROST shares, mesh identities, Vaultwarden's own keys) is generated by the keep binaries or by Vaultwarden, which have their own guarantees.
