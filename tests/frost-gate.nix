...
{ ... }:
{
  ...
  testScript = ''
    ...
    node.start()
    node.wait_until_succeeds("systemctl is-failed --quiet keep-node-frost-gate.service")
    # wait_until (not a bare grep): same journal-flush race as above; the unit can be failed a
    # moment before its final log line lands. A genuinely wrong failure path still fails here.
    node.wait_until_succeeds(
        "journalctl -b -u keep-node-frost-gate.service | grep -q 'OPRF-provisioned'"
    )
